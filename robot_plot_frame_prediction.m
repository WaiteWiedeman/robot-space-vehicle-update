function h1=robot_plot_frame_prediction(sysParams,Ycg,Xcg,alph,xend0,yend0,xend1,yend1,xend2,yend2)
    % True system
    b1 = sysParams.b1/2;
    b2 = sysParams.b2/2;
    r = sqrt(b1^2+b2^2);
    ...
    phi4 = atan2(-b2,b1);
    % cart
    patch('XData',Xcg+[r*cos(phi1+alph) r*cos(phi2+alph) r*cos(phi3+alph) r*cos(phi4+alph)], ...
        'YData',Ycg+[r*sin(phi1+alph) r*sin(phi2+alph) r*sin(phi3+alph) r*sin(phi4+alph)],...
        'FaceColor','none', 'FaceAlpha', 0, ...
        'EdgeColor','r','LineWidth',2,'LineStyle','--'); %,'FaceAlpha', 0 ,'k' 'FaceColor','#808080',
    % plots rod and blob
    h1 = plot([xend0 xend1],[yend0 yend1],'r','LineWidth', 3, 'LineStyle','--', "DisplayName", "Prediction");
    plot(xend0,yend0,'Marker','o','MarkerSize',12,'MarkerEdgeColor','r');
    plot(xend1,yend1,'Marker','o','MarkerSize',12,'MarkerEdgeColor','r');
    plot([xend1 xend2],[yend1 yend2],'r','LineWidth', 3, 'LineStyle','--');
    plot(xend2,yend2,'Marker','o','MarkerSize',12,'MarkerEdgeColor','r');
